...
fn populate_file_maps(_bpf: &mut aya::Ebpf, _policy: &Policy) -> anyhow::Result<()> {
    // TODO: path prefix map. Kernel-side program reads only; userspace here
    // will serialise the prefix strings into fixed-size records once the
    // kernel program lands.
    Ok(())
}
