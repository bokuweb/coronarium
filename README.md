...
| **Workspace poisoning (folderOpen task)** | A repo with `.vscode/tasks.json` auto-running on `folderOpen` | IOC catalog v2026.05.21+ ships `vscode.tasks-folderopen-autorun` as a basename-scoped content needle (High severity, family `editor-extension`) #23 |
| **Workspace poisoning (terminal autorun)** | A repo whose `.vscode/settings.json` (or `.cursor/settings.json`) declares a terminal profile whose `args` co-occur a shell-exec flag (`-c`, `/c`, `-Command`) and a downloader / dynamic-evaluator token (`curl`, `wget`, `iwr`, `Invoke-Expression`, `eval`, `base64 -d`) — opening a terminal auto-runs the downloaded payload | IOC catalog v2026.05.27+ ships `editor-extension.terminal-profile-autorun-downloader` as a structural JSONC rule scoped to workspace settings (Medium severity); the rule parses settings.json with a small inline JSONC normaliser and demands the co-occurrence inside the *same* profile so legitimate `bash --login` profiles don't trip it #27 |
